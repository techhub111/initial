...
public class RetailManagerServiceImpl implements RetailManagerService{
	...
	public Shop create(GeocodeRequest geocodeRequest) {
		...
		if(null != existingShop)
		{
			...
			{
				String uri = ceateURI(geocodeRequest);
	...
	@Override
	@Transactional
	public Shop getShopWithinMinimumDistance(LatLng latLng, Iterable<Shop> shopsList) {
		return retailManagerDao.getShopWithinMinimumDistance(latLng, shopsList);
	}


	/*
	 * This method creates URI, which takes base path of Google Geocoding API, address and key parameters as input.
	 */
	@Override
	public String ceateURI(GeocodeRequest geocodeRequest) {
		return new StringBuffer(environment.getProperty("basePath")).append(geocodeRequest.getShopNumber())
				.append(environment.getProperty("addressJoiner")).append(geocodeRequest.getPostalCode())
				.append(environment.getProperty("keyJoiner")).append(environment.getProperty("geoCodeApiKey"))
				.toString();
	}
